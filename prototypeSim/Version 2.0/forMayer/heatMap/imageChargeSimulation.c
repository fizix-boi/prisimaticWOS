...
int main(void){
	...
	double dpf = 1;
	double num = 5;
	double ddp = (dpf - dp0) / (num - 1);
	
	double t0 = 0;
	double tf = 1;
	double numt = 5;
	double dt = (tf - t0) / (numt - 1);
	...
	for(; l < numt; l++){
		printf("Top Metal Voltage (mV):\t%f\n", ((&(objects[0]))->V) * 1000.0);
		
		fprintf(printer, "Top Metal Voltage (mV):\t%f\n", ((&(objects[0]))->V) * 1000.0);
		fprintf(printer, "Dot Voltage (mV)\tDot Voltage (meV/e)\tST Voltage (meV/e)\n");
		...
		for(; j < num; j++){
			...
			((&epectron)->pos)[0] = 0.500001 * dtd;
			double dot = 0;
			i = 0;
			
			for(; i < reps; i++){
				dot += potentialShot(epectron, objects, numObs, lBound, hBound);
				if(((i + 1) % (reps / div)) == 0){
					printf("X");
				}
			}
			...
			((&epectron)->pos)[0] = 2.0 * lx;
			double edge = 0;
			i = 0;
			
			for(; i < reps; i++){
				edge += potentialShot(epectron, objects, numObs, lBound, hBound);
				if(((i + 1) % (reps / div)) == 0){
					printf("X");
				}
			}
			...
			double dotConf = dot - base;
			double stConf = base - edge;
			
			fprintf(printer, "%f\t%f\t%f\n", ((&(objects[1]))->V) * 1000.0, dotConf * -1000.0, stConf * -1000.0);
...
double potentialShot(ElectronClass charge, MetalSheetClass* objects, int numObs, double lBound, double hBound){
	...
	while(true){
		...
		if((r < lBound) || (r > hBound)){
			double eStatConst = 0.5 * ((&charge)->Q) / (4 * M_PI * e0);
