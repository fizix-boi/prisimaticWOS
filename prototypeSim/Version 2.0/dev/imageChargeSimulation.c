...
int main(void){
	...
	double lz = 40 * pow(10, (-9));
	double dr = (50 * pow(10, (-9)));
	double d = dr + (0.5 * lz);
	...
	int lead1NumHoles = 1;
	HoleClass lead1Holes[lead1NumHoles];
	double lead1H0Pos[] = {0.0, 0.0, 0.0}; //note that this last arg wont matter
	hole(&(lead1Holes[0]), lead1H0Pos, 100 * pow(10, (-9)));
	
	double lead1Pos[] = {0.0, 0.0, 0.0};
	double lead1Geo[] = {lx, ly, lz};
	metalSheet(&lead1, 0, lead1Pos, lead1Geo, lead1NumHoles, lead1Holes);
	objects[0] = lead1;
	...
	*/
	...
	volt *= normer;
	printf("V = %f", volt / (((&epectron)->Q) / (4 * M_PI * e0 * 2 * dr)));
...
void findClosestPoint(double* closestPoint, double* posUse, MetalSheetClass objecti){
	...
	if(!holy){
		...
		if(!isIn){
			holyLoc[2] = halfGeom[2];
		}else{
			holyLoc[2] = norm[2];
		}
		add3Arr(closestPoint, holyLoc, center);
	}
}
